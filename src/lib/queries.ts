...
export const saveActivityLogsNotifications = async ({
    ...
    if(!foundAgencyId){
        if(!subaccountId){
            throw new Error(
                'You need to provide atleast an agencyId aur subaccount Id'
            )
        }
...
export const upsertAgency = async (agency: Agency) => {
  ...
  // getting all the subaccounts, which the user has permission for
  ...
  export const updateLanesOrder = async (lanes: Lane[]) => {
    try {
        const updateTrans = lanes.map((lane) => db.lane.update({
            where: {
                id: lane.id
            },
            data: {
    ...
    export const updateTicketsOrder = async (tickets: Ticket[]) => {
        try {
            ...
            console.log("Failed To update ticket order. ⭕")
        }
    }
    ...
    export const upsertFunnelPage = async (
        ...
        const reponse = await db.funnelPage.upsert({
            where: {
                id: funnelPage.id || ''
            },
            update: { ...funnelPage },
