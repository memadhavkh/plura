...
const UserDetails = ({ id, type, subAccounts, userData }: Props) => {
  ...
  const onChangePermission = async (
      ...
      await saveActivityLogsNotifications({
        ...
        description: `Gave ${userData?.name} access to | ${
          subAccountPermissions?.Permissions.find(
            (p: Permissions) => p.subAccountId === subAccountId
          )?.SubAccount.name
        } `,
    ...
    if (response) {
      ...
      if (subAccountPermissions) {
        subAccountPermissions.Permissions.find((perm: Permissions) => {
  ...
  const onSubmit = async (values: z.infer<typeof userDataSchema>) => {
    ...
    if (userData || data?.user) {
      ...
      authUserData?.Agency?.SubAccount.filter((subaccount: SubAccount) =>
        authUserData.Permissions.find(
          (p: Permissions) => p.subAccountId === subaccount.id && p.access
        )
                ...
                <div className="flex flex-col gap-4">
                  {subAccounts?.map((subAccount) => {
                    const subAccountPermissionsDetails =
                      subAccountPermissions?.Permissions.find(
                        (p: Permissions) => p.subAccountId === subAccount.id
                      )
                    return (
                      <div
                        key={subAccount.id}
